...
class newspaper{
    ...
    int publicationYear;
    int price;
    newspaper(int a,String b,int c,int d){
        this.regNo=a;
        this.name=b;
        this.publicationYear=c;
        this.price=d;
    }
    int getregNo(){
        return regNo;
...
class IPA240{
    public static void main(String args[]){
        ...
        int ans1=findTotalPriceByPublicationYear(arr, year);
        if(ans1>0){
            System.out.println(ans1);
        }else{
            System.out.println("No Newspaper found with the mentioned attribute");
        }
        newspaper ans2=searchNewspaperByName(arr, name);
        if(ans2!=null){
            System.out.println(ans2.getregNo());
            System.out.println(ans2.getname());
            System.out.println(ans2.getpublicationYear());
            System.out.println(ans2.getprice());
        }else{
            System.out.println("No Newspaper found with the given name");
        }
    }
    public static int findTotalPriceByPublicationYear(newspaper[] arr,int year){
        int sum=0;
        for(int i=0;i<arr.length;i++){
